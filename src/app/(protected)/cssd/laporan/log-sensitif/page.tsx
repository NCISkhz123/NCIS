...
export default async function CssdSensitiveLogPage() {
  const supabase = await createServerSupabaseClient();
  const { data: { user } } = await supabase.auth.getUser();
  const role = user?.user_metadata?.role || user?.app_metadata?.role;
  
  if (role !== "KEPALA_SEKSI") {
    redirect("/(protected)/cssd/laporan");
  }

  const logs = await listDeletedTransactionLogs(supabase, "CSSD");
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Riwayat penghapusan transaksi pada modul CSSD. Hanya dapat diakses oleh Superadmin.
        </p>
      </div>
