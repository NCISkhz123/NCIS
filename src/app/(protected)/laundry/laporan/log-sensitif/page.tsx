...
export default async function LaundrySensitiveLogPage() {
  const supabase = await createServerSupabaseClient();
  const { data: { user } } = await supabase.auth.getUser();
  const role = user?.user_metadata?.role || user?.app_metadata?.role;
  
  if (role !== "KEPALA_SEKSI") {
    redirect("/(protected)/laundry/laporan");
  }

  const logs = await listDeletedTransactionLogs(supabase, "LAUNDRY");
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Riwayat penghapusan transaksi pada modul Laundry. Hanya dapat diakses oleh Superadmin.
        </p>
      </div>
