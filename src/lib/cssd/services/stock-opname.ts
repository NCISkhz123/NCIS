...
import type { CssdRpcClient, ServiceResult } from "@/lib/cssd/services/stock";
...
export async function createDraftStockOpnameSession(
  ...
  return executeOpnameRpc<StockOpnameCreateResult>(
    client,
    "cssd_create_stock_opname_session",
    {
...
export async function finalizeStockOpnameSession(
  ...
  return executeOpnameRpc<StockOpnameFinalizeResult>(
    client,
    "cssd_finalize_stock_opname_session",
    {
      p_session_id: parsed.data.sessionId,
    }
  );
...
export async function getDraftStockOpnameSession(
  ...
  const { data, error } = await supabase
    ...
    .select("id, opname_date, status, notes, stock_opname_lines(id)")
    .eq("status", "DRAFT")
...
async function getCurrentBalance(
  ...
  const { data, error } = await supabase.rpc("cssd_get_balance", {
    p_item_id: itemId,
...
export async function listStockOpnameLines(
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, item_id, stock_position, hospital_unit_id, counted_quantity, notes, items!inner(id, code, name, item_type, is_active), hospital_units(name)"
    )
