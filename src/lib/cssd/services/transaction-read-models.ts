...
export async function listRecentTransactionHistory(
  ...
  if (error) {
    console.error("Failed to fetch CSSD transaction history:", error);
    return [];
  }
...
export async function listStockSummary(
  ...
  let query = supabase
    .from("stock_balances")
    .select(
      "item_id, stock_position, quantity, hospital_unit_id, items!inner(id, code, name, item_type, is_active), hospital_units(name)"
    )
    .gt("quantity", 0)
  ...
  if (error) {
    console.error("Failed to fetch CSSD stock summary:", error);
    return [];
  }
