...
import { generateItemCode } from "@/lib/laundry/codegen/item-code";
...
export type MasterDataTable =
  | "laundry_units_of_measure"
  | "laundry_hospital_units"
  | "laundry_items";
...
async function getItemsByType(
  ...
  const { data, error } = await client.findMany<ItemRow>("laundry_items", {
...
export async function listHospitalUnits(
  ...
  const { data, error } = await client.findMany<HospitalUnitRow>(
    "laundry_hospital_units",
    {
    orderBy: {
      column: "name",
...
export async function createItem(
  ...
  const { data, error } = await client.insertOne<ItemRow>("laundry_items", {
...
export async function updateItem(
  ...
  const { data, error } = await client.updateById<ItemRow>("laundry_items", id, {
    code: codeResult.data,
    item_type: parsed.data.itemType,
    name: parsed.data.name,
    uom_id: parsed.data.uomId,
