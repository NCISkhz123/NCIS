...
export async function listRecentTransactionHistory(
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, movement_type, from_position, to_position, hospital_unit_id, quantity, notes, occurred_at, laundry_items!inner(id, code, name, item_type, is_active), laundry_hospital_units(name)"
    )
