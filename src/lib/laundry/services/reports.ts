...
type StockPosition = keyof typeof LAUNDRY_STOCK_POSITION_LABELS;
type MovementType =
  | "RECEIPT"
  | "DISTRIBUTION"
  | "RETURN"
...
const LAUNDRY_INTERNAL_POSITIONS = new Set<StockPosition>([
  "READY",
  "NON_STERILE",
  "STERILIZATION_AREA",
]);
...
function toPositionLabel(position: StockPosition | null) {
  return position ? LAUNDRY_STOCK_POSITION_LABELS[position] : null;
}
...
function resolveDisplayUnitName(row: {
  ...
  if (
    (row.stock_position && LAUNDRY_INTERNAL_POSITIONS.has(row.stock_position)) ||
    (row.from_position && LAUNDRY_INTERNAL_POSITIONS.has(row.from_position)) ||
    (row.to_position && LAUNDRY_INTERNAL_POSITIONS.has(row.to_position))
  ) {
    return "Laundry";
  }
